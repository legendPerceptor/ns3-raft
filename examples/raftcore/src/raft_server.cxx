...
raft_server::raft_server(context* ctx)
    ...
    */
    ...
    //How to change the commiting_thread to make it work in NS3?
...
void raft_server::handle_peer_resp(ptr<resp_msg>& resp, const ptr<rpc_exception>& err) {
    ...
    l_->debug("We are in the handle peer_resp~");
    ...
    // update peer last response time
    {
        ...
        if (peer != peers_.end()) {
            l_->debug("before system time");
#ifdef NS3_ENV
            l_->debug(sstrfmt("system now ms:%d").fmt(system_clock::Now().GetMilliSeconds()));
...
void raft_server::become_leader() {
    ...
    if (ctx_->event_listener_)
    {
        ...
        l_->debug(sstrfmt("server %d becomes leader").fmt(ctx_->state_mgr_->server_id()));
    }
...
void raft_server::become_follower() {
    ...
    if (ctx_->event_listener_)
    {
        ...
        l_->debug(sstrfmt("server %d becomes follower").fmt(ctx_->state_mgr_->server_id()));
    }
}
        ...
        try {
            while (quick_commit_idx_ <= sm_commit_index_
                ...
                if (ctx_->event_listener_)
                {
                    ...
                    l_->debug(sstrfmt("server %d logs_catch_up").fmt(ctx_->state_mgr_->server_id()));
                }
            ...
            while (sm_commit_index_ < quick_commit_idx_ && sm_commit_index_ < log_store_->next_slot() - 1) {
                sm_commit_index_ += 1;
                l_->debug(sstrfmt("sm_commit_index(%d), quick_comit_idx(%d), log_store_next(%d)").fmt(sm_commit_index_,quick_commit_idx_,log_store_->next_slot()));
                ...
                if (log_entry->get_val_type() == log_val_type::app_log) {
                    ...
                    l_->debug(sstrfmt("CommitInBG-%s").fmt(reinterpret_cast<const char *>(log_entry->get_buf().data())));
            ...
            std::cerr<<"INTO EXCEPTION IN BG COMMIT"<<std::endl;
...
#ifdef NS3_ENV
        time_elasped_since_quorum_resp = static_cast<int32>(Simulator::Now().GetMilliSeconds() - peer_resp_times[peers_.size() / 2].GetMilliSeconds());
        l_->debug(sstrfmt("time elasped:%d").fmt(time_elasped_since_quorum_resp));
#else
        time_elasped_since_quorum_resp = static_cast<int32>(std::chrono::duration_cast<std::chrono::milliseconds>(
                system_clock::now() - peer_resp_times[peers_.size() / 2]).count());
#endif
