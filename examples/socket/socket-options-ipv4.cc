...
class my_rpc_session {

private:
    int count;
    Ptr<Socket> socket;
    void HandleRead(Ptr<Socket> socket){
        std::cout<<"Read packets! "<<Ipv4Address::GetAny ()<<std::endl;
    }

public:
    void start(){
        socket->SetRecvCallback(MakeCallback(&my_rpc_session::HandleRead,this));
    }
    my_rpc_session(Ptr<Socket> socket_, int count_):socket(socket_),count(count_){}
};
...
//
  ...
  my_rpc_session rpc_session(recvSink,0);
  rpc_session.start();
